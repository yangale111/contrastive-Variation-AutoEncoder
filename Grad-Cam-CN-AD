...
# fake_data_nums = 100
attMAPs = []
cnn_weights='./result/tf_weights_mae_CN_AD_4/model_99-0.93.h5'
learning_rate = 0.0001
model = Mynet(learning_rate)
model.load_weights(cnn_weights)
grad_cam_plus_plus = GradCAMPlusPlus3D(model, 0, layer_name='aspp_outputs_2')
while k < 5:
    print(k)
...
def get_mask_corr_nii(str,corr_nii):
    ...
    # corr_nii_mask = np.where(mask_n < 170, corr_nii, 0)
    ...
    corr_nii_mask[corr_nii_mask  <  0.7*np.max(corr_nii_mask)] = 0
...
salient_map = get_mask_corr_nii('./roi_result/CN_AD_salient_map_2.nii.gz',mean_attmap)
...
get_top_salient_map(salient_map,[80,78,42,46],'./roi_result/CN_AD_top_salient_map.nii.gz')
