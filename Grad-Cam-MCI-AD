...
def grad_cam_plus(model, img, layer_name="block5_conv3", label_name=None, category_id=None):
    img_tensor = img  # (1,182,218,182,1)
...
def compute_heatmap(model, img, conv,activation='sigmoid', pred_index=0):
    """
    ...
    grads = tf.squeeze(tape.gradient(class_channel, conv_output))
    ...
    heatmap = resize(heatmap, (91, 109, 91))
    # print(heatmap.shape)
    return heatmap


def Get_Build_model(model,Layer_name):
    Build_model=tf.keras.models.Model([model.inputs], [model.get_layer(Layer_name).output, model.output])
    return Build_model


def Guided_GradCAM_3D(Grad_model,ct_io,Class_index = 1):
    # Create a graph that outputs target convolution and output
    grad_model = Grad_model
    # input_ct_io=tf.expand_dims(ct_io, axis=-1)
    # input_ct_io=tf.expand_dims(input_ct_io, axis=0)
    with tf.GradientTape() as tape:
        conv_outputs, predictions = grad_model(ct_io)
...
def generate_guided_grad_cam(model,image,Layer_name):
    # Reading the CT
    get_grad_model=Get_Build_model(model,Layer_name)

    heatmap=Guided_GradCAM_3D(get_grad_model, image,Class_index=0)
    # print(heatmap)
    return heatmap
...
# fake_data_nums = 100
attMAPs = []
cnn_weights='./result_1/tf_weights_mae_MCI_AD_0/model_154-0.80.h5'
learning_rate = 0.0001
model = Mynet(learning_rate)
model.load_weights(cnn_weights)
grad_cam_plus_plus = GradCAMPlusPlus3D(model, 0, layer_name='conv4')
while k < 5:
    print(k)
    # cnn_weights=glob('./result/tf_weights_mae_MCI_AD_{}/*'.format(k))
...
attMAPs = np.reshape(attMAPs,(-1,91,109,91))
mean_attmap = np.mean(attMAPs,axis=0)


def get_mask_corr_nii(str,corr_nii):
    mask = ants.image_read('/root/commonfiles/wangyuqi/Sex/BN_Atlas_246_2mm.nii.gz')
    mask_n = mask.numpy()
...
salient_map = get_mask_corr_nii('./roi_result/MCI_AD_salient_map_3.nii.gz',mean_attmap)
...
def get_the_positive_areas(corr_nii_mask):
    ...
    print(map)
    indices = list(map.keys())
    values = list(map.values())
    # 绘制折线图
    plt.plot(indices, values, marker='o', linestyle='-')
    plt.xlabel('Index')
    plt.ylabel('Value')
    plt.title('Line Plot of Values by Index')
    plt.grid(True)
    plt.show()
...
get_top_salient_map(salient_map,[50,70,48,54,56,41],'./roi_result/MCI_AD_top_salient_map.nii.gz')
