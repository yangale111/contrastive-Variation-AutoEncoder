...
fake_dir = './Dataset/fake_data_CN_AD/'
...
def create_fake_dataset(z_encoder, s_encoder, train_data, batch_size, k):
    # 获得特征分布，提取特征，为了增加噪声，给特征加上正态分布的随机噪声
    z_features = []
    s_features = []
    ...
    # 创建假数据的内部函数，通过组合z特征和s特征，使用解码器生成假数据，对生成的假数据进行配准，使其与模板图像对其
    ...
    # 生成并保存假数据，迭代生成假数据，分别生成背景数据和目标数据，背景数据用z_vector和零向量生成，目标数据使用z_vector和s_vector生成，并将生成的数据保存到相应的目录中，并返回背景数据和目标数据
...
while k < 5:
    ...
    CN_train_data_nums = CN_train_data.shape[0]
    fake_background_data, fake_target_data = create_fake_dataset(z_encoder, s_encoder, AD_train_data,batch_size, k)

    create_tfrecord(CN_train_data, AD_train_data, fake_background_data, fake_target_data, fake_dir+'train_{}/'.format(k) + 'aug_train.tfrecord')
    create_tfrecord(CN_train_data, AD_train_data,[],[], fake_dir+'train_{}/'.format(k) + 'ori_train.tfrecord')
    create_tfrecord(CN_test_data, AD_test_data, [],[], fake_dir+'train_{}/'.format(k) + 'test.tfrecord')
