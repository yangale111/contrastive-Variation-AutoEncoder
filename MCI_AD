...
while k < 5:
    ...
    # 数据合并
    train_data = np.concatenate((MCI_train_data,AD_train_data), axis=0)
    ...
    for i in tqdm(range(1, nbatches)):
        ...
        hist = cvae.train_on_batch([ad_batch, mci_batch])
        loss.append(hist[0])
        if pre_hist <= hist[0]:
            patience += 1
        else:
        ...
        if patience == 30 and lr > 0.000001:
            ...
            print('new lr:',lr)
            pre_hist = hist[0]
        mse = ((np.array([ad_batch, mci_batch]) - np.array(cvae.predict([ad_batch,mci_batch]))[:, :, :, :, :,0])**2).mean()
        ...
        if np.mod(i, 5) == 0:  # Plot training progress
            ...
            plot_four(ad_batch, mci_batch, z_encoder, s_encoder, cvae_decoder, cvae, idx=0)
            ...
            test_mse = ((np.array([t_ad_batch, t_mci_batch]) - np.array(cvae.predict([t_ad_batch, t_mci_batch]))[:, :, :, :, :,0])**2).mean()
