...
_ = os.mkdir('./result_1/') if not os.path.exists('./result_1/') else 0
for j in range(10):
    k = 0
    # fake_data_nums = 100
    while k < 5:
        ...
        batch_size = 64
        epoch = 1000
        train_steps = ((len(CN_trainset_filepath)+len(MCI_trainset_filepath))) // batch_size
        val_steps = len(CN_testset_filepath) + len(MCI_testset_filepath)
        ...
        _ = os.mkdir('./result_1/tf_weights_mae_CN_MCI_{}/'.format(k)) if not os.path.exists('./result_1/tf_weights_mae_CN_MCI_{}/'.format(k)) else 0
        cnn_file_path='./result_1/tf_weights_mae_CN_MCI_{}/'.format(k)+"model_{epoch:02d}-{val_accuracy:.2f}.h5"
        callbacks_list = [
        ...
        model.save_weights('./result_1/tf_weights_mae_CN_MCI_{}/'.format(k)+'model_{}.h5'.format(j))
...
for j in range(10):
    ...
    while k < 5:
        ...
        MCI_trainset_filepath = [MCI_dataset_filepath[i] for i in MCI_flods[k][0]]
        MCI_testset_filepath = [MCI_dataset_filepath[i] for i in MCI_flods[k][1]]
        print(len(CN_trainset_filepath),len(MCI_trainset_filepath))
        print(len(CN_testset_filepath),len(MCI_testset_filepath))
        batch_size = 32
        epoch = 200
        train_steps = (len(CN_trainset_filepath)+len(MCI_trainset_filepath)) // batch_size
        val_steps = (len(CN_testset_filepath) + len(MCI_testset_filepath))
        ...
        model = Mynet(learning_rate)
        _ = os.mkdir('./result_1/tf_weights_mae_CN_MCI_ori_{}/'.format(k)) if not os.path.exists('./result_1/tf_weights_mae_CN_MCI_ori_{}/'.format(k)) else 0
        cnn_file_path='./result_1/tf_weights_mae_CN_MCI_ori_{}/'.format(k)+"model_{epoch:02d}-{val_accuracy:.2f}.h5"
        callbacks_list = [
            callbacks.ReduceLROnPlateau(
             # This callback will monitor the validation loss of the model
             monitor='accuracy',
        ...
        model.fit(train_dataset,batch_size = batch_size,
                  validation_data=test_dataset,validation_batch_size=1,
                  steps_per_epoch=train_steps,validation_steps=val_steps,shuffle=True,epochs=epoch,callbacks=callbacks_list)
        model.save_weights('./result_1/tf_weights_mae_CN_MCI_ori_{}/'.format(k)+'model.h5')
        k+=1
        tf.compat.v1.reset_default_graph()
...
model = Mynet(0.0001)
weights = sorted(glob('/root/commonfiles/wangyuqi/Sex/ADNI_CVAE/result_1/tf_weights_mae_CN_MCI_ori_{}/*'.format(k)))
...
while k < 5:
    cnn_weights = sorted(glob('./result_1/tf_weights_mae_CN_MCI_ori_{}/*'.format(k)))
    # print(cnn_weights)
    CN_testset_filepath = [CN_dataset_filepath[i] for i in CN_flods[k][1]]
    MCI_testset_filepath = [MCI_dataset_filepath[i] for i in MCI_flods[k][1]]
    print(len(CN_testset_filepath), len(MCI_testset_filepath))
...
model = Mynet2(learning_rate)
while k < 5:
    cnn_weights=sorted(glob('./result_1/tf_weights_mae_CN_MCI_{}/*'.format(k)))
    # print(cnn_weights)
    CN_testset_filepath = [CN_dataset_filepath[i] for i in CN_flods[k][1]]
    MCI_testset_filepath = [MCI_dataset_filepath[i] for i in MCI_flods[k][1]]
    print(len(CN_testset_filepath),len(MCI_testset_filepath))
    val_steps = (len(CN_testset_filepath) + len(MCI_testset_filepath))
    test_dataset = load_tf_data(fake_dir+'train_{}/'.format(k) + 'test.tfrecord',1,1,False)
    model.load_weights(cnn_weights[-1])
    for x, y in test_dataset:
